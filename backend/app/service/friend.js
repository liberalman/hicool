...
class FriendService extends Service {
  ...
  async create(uid) {
    ...
    try {
      const article = await ctx.model.Friend.create(ctx.request.body)
      ctx.status = 201
      ctx.body = {
  ...
  async delete(uid, id) {
    let { ctx } = this
    const article = await ctx.model.Friend.findOne({
      _id: id
    })
    if (!article) {
      ctx.status = 500
    ...
    if(article.author_id.toString() != uid) {
      ...
      try {
        await ctx.model.Friend.findByIdAndRemove(id)
  ...
  async update(uid, id, data) {
    ...
    if(!article || article.author_id.toString() != uid) {
      ...
      try {
        const article = await ctx.model.Friend.findByIdAndUpdate(id, data, {
          new: true
        })
        ctx.status = 200
        ctx.body = {
  ...
  async find(id) {
    ...
    //每次获取之后,将阅读数加1
    try {
      let result = await ctx.model.Friend.findById(id, '-images').populate({
        path: 'author_id',
      ...
      result.visit_count++
      await ctx.model.Friend.findByIdAndUpdate(id, {
        $inc: {
          visit_count: 1
        }
      })
  ...
  async search() {
    ...
    try {
      const list = await ctx.model.Friend.find(condition)
        ...
        .sort(sortName)
        .exec()
      const total = await ctx.model.Friend.countDocuments(condition)
      return {
        list: list,
        'total': total,
        'page': page,
  ...
  async list(uid) {
    ...
    let offset = (page - 1) * size
    try {
      const queryTotal = await ctx.model.Friend.aggregate([{
        "$match": {
          "_id": uid,
          "friends": {
            $exists: true
      ...
      const userIds = await ctx.model.Friend.findOne({
          "_id": uid
        }, {
          "friends": {
            "$slice": [offset, size]
      ...
      const list = await ctx.model.User.find({
        ...
        .select('_id nickname avatar description')
      ctx.body = {
        'list': list,
  ...
  async toggleLike(uid) {
    ...
    try {
      ...
      const article = await ctx.model.Friend.findByIdAndUpdate(articleId, conditionTwo, {
        new: true
      })
      ctx.status = 200
      ctx.body = {
