...
export default {
  ...
  methods: {
    getBaiduToken() {
      // 第一步获取token
      let _this = this
      axios.get('https://aip.baidubce.com/oauth/2.0/token?grant_type=client_credentials&client_id=lHEhnp7KXT1gQTIbxlaIWG2G&client_secret=qxCn1ar7klqbooh9ds6LOQbQXyMlK8h4',{headers:{
        dataType:'json'
      }}).then(res =>{
        if(res.status ==200){
