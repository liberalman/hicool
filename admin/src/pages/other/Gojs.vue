<template>
  <div class="goDemo">
    <div id="mygoChart" style="width:1000px; height:600px; background-color: #ECA;">
    </div>
    <button @click="toBig">
      放大
    </button>
    <button @click="toSmall">
      缩小
    </button>
    <button @click="toFilt">
      自动
...
<script>
  ...
  export default {
    mounted() {
      ...
      mySelf.myDiagram.addDiagramListener("ObjectSingleClicked",
        function(e) {
          debugger
      ...
      mySelf.myDiagram.addDiagramListener("ClipboardPasted",
        function(e) {
          debugger
      ...
      // 定义个简单的 Node 模板
      mySelf.myDiagram.nodeTemplate = MAKE(go.Node, new go.Binding("location", "loc", go.Point.parse), MAKE(go.Shape, "RoundedRectangle", {
          fill: "#44CCFF",
          stroke: 'green',
          strokeWidth: 2,
          angle: 15
        ...
        MAKE(go.Panel, "Horizontal", {
          ...
          MAKE(go.Panel, "Vertical", MAKE(go.Picture, {
              margin: 10,
              width: 50,
              height: 50,
              background: "red"
            },
            new go.Binding("source", "img"))), MAKE(go.Panel, "Vertical", MAKE(go.TextBlock, "Default Text", {
              margin: 12,
              stroke: "white",
              font: "bold 16px sans-serif",
            },
            new go.Binding("text", "name1")), MAKE(go.TextBlock, {
              stroke: "red"
            },
            {
              margin: 5
            },
            new go.Binding("text", "name2")), MAKE(go.TextBlock, {
        ...
        },
        {
          toolTip: MAKE(go.Adornment, "Spot",
            ...
            MAKE(go.Shape, "RoundedRectangle", {
              ...
              new go.Binding("text", "name1"))) // end of Adornment
        });
      mySelf.myDiagram.linkTemplate = MAKE(go.Link,
        ...
        MAKE(go.TextBlock, {
          ...
          toolTip: MAKE(go.Adornment, "Auto", MAKE(go.Shape, {
            fill: "#FFFFCC"
          }), MAKE(go.TextBlock, {
              margin: 4
            },
